...
const ICE_DEMON_MAP_DEFINITION: MapDefinition = {
  baseX: 3300,
  baseY: 5344,
  width: 28,
  height: 26,
...
export default function IceDemonPage() {
  ...
  const {
    ...
    npcState,
    loading,
  } = useStageEvents<CoxRaid>(Stage.COX_ICE_DEMON);
  ...
  const iceDemonData = challenge.coxRooms.iceDemon;
  if (challenge.status !== ChallengeStatus.IN_PROGRESS && iceDemonData === null) {
    return <>No Ice Demon data for this raid</>;
  }
  ...
  return (
    <>
      <div className={bossStyles.overview}>
        <BossFightOverview
          name="Ice Demon"
          image="/images/cox/ice_demon.png"
          time={totalTicks}
          sections={stats}
        />
      </div>
      ...
      <div className={bossStyles.charts}>
        <Card
          ...
          header={{ title: "Ice Demon's Health By Tick" }}
        >
          <BossPageDPSTimeline
            currentTick={currentTick}
