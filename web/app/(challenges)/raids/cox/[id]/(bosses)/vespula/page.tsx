...
const VESPULA_MAP_DEFINITION: MapDefinition = {
  baseX: 3331,
  baseY: 5283,
  width: 32,
  height: 30,
...
export default function VespulaPage() {
  ...
  const {
    ...
    npcState,
    loading,
  } = useStageEvents<CoxRaid>(Stage.COX_VESPULA);
  ...
  const bossHealthChartData = useMemo(() => {
    let abyssalPortal: EnhancedRoomNpc | null = null;
    const iter = npcState.values();
    for (let npc = iter.next(); !npc.done; npc = iter.next()) {
      if (Npc.isAbyssalPortal(npc.value.spawnNpcId)) {
        abyssalPortal = npc.value;
        break;
      }
    }

    if (abyssalPortal !== null) {
      return abyssalPortal.stateByTick.map((state, tick) => ({
        tick,
    ...
    for (const event of events) {
      if (
        ...
        event.npc !== undefined &&
        Npc.isAbyssalPortal(event.npc.id)
      ) {
        healthByTick.set(
          event.tick,
  ...
  const vespulaData = challenge.coxRooms.vespula;
  if (challenge.status !== ChallengeStatus.IN_PROGRESS && vespulaData === null) {
    return <>No Vespula data for this raid</>;
  }
    ...
    <>
      ...
      <div className={bossStyles.charts}>
        <Card
          ...
          header={{ title: "Abyssal Portal's Health By Tick" }}
        >
          <BossPageDPSTimeline
            currentTick={currentTick}
