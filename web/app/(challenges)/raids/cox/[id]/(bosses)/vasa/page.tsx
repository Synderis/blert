...
const VASA_MAP_DEFINITION: MapDefinition = {
  baseX: 3300,
  baseY: 5280,
  width: 28,
  height: 28,
...
export default function VasaPage() {
  ...
  const bossHealthChartData = useMemo(() => {
        ...
        for (let npc = iter.next(); !npc.done; npc = iter.next()) {
          if (Npc.isVasaNistirio(npc.value.spawnNpcId)) {
            vasa = npc.value;
            break;
          }
        }
        ...
        if (vasa !== null) {
          return vasa.stateByTick.map((state, tick) => ({
            tick,
        ...
        for (const event of events) {
          if (
            ...
            event.npc !== undefined &&
            Npc.isVasaNistirio(event.npc.id)
          ) {
            healthByTick.set(
              event.tick,
...
const getEntities = useMapEntities(
  ...
  const vasaData = challenge.coxRooms.vasa;
  if (challenge.status !== ChallengeStatus.IN_PROGRESS && vasaData === null) {
    return <>No Vasa Nistirio data for this raid</>;
  }
    ...
    <>
      ...
      <div className={bossStyles.charts}>
        <Card
          ...
          header={{ title: "Vasa Nistirio's Health By Tick" }}
        >
          <BossPageDPSTimeline
            currentTick={currentTick}
